...
public class ParserTest extends BaseTest {
    ...
    private expression.parser.Either<Reason, Integer> eval(final LongSupplier supplier) {
        try {
            return lift(supplier.getAsLong());
        } catch (final ExpException e) {
            return expression.parser.Either.left(e.reason);
        }
    }
    ...
    protected TripleExpression parse(final String expression, final boolean reparse) {
        ...
        try {
            final UpgradedParser parser = new UpgradedParser();
            if (reparse) {
                counter.nextTest();
    ...
    private void check(final int[] vars, final TripleExpression expression, final expression.parser.Either<Reason, Integer> answer) {
        counter.nextTest();
        op();
        try {
    ...
    private Test constOrVariable(final int[] vars) {
        if (random.nextBoolean()) {
            ...
            return new Test("xyz".charAt(id) + "", expression.parser.Either.right(vars[id]));
        } else {
            final int value = random.nextInt();
            return new Test(value + "", expression.parser.Either.right(value));
        }
    }
    ...
    protected expression.parser.Either<Reason, Integer> lift(final long value) {
        return expression.parser.Either.right((int) value);
    }

    protected static class Test {
        final String expr;
        final expression.parser.Either<Reason, Integer> answer;

        Test(final String expr, final expression.parser.Either<Reason, Integer> answer) {
            this.expr = expr;
            this.answer = answer;
        }
    }
